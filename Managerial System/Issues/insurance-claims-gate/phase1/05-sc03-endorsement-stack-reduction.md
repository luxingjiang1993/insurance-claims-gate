...
- [x] SC-03 夹具经 HTTP：批单缩责 → 减赔草案，引用服从效力栈
- [x] citations 含 `authority_rank`；批单覆盖主险时可观察 `overridden_by`（或等价字段）
- [x] `calc_steps` 可复核；条款冲突时失败关闭，不静默改金额
- [x] `endorsement_priority` 画像下先查批单再主险，可被 machine_check / 黑盒路径验证
- [x] 减赔导出复用 citations 与 calc_steps；金额档人闸细节可留给 06，但无人闸前不得谎称出款就绪
- [x] SC-03 轨 A machine_check 稳定绿
- [x] handoff 含 `Rewrote from: REF-CASE-KB, REF-MISSIONS, REF-COURSE-04`

## Answer

SC-03 已在轨 A 落地并通过机检：夹具 `CLM-SC03-001`（材料齐 + `endorsement_flags=["PA-ACC-END-001"]`）→ `decision_type=reduce` / `gate_status=ADJUSTING`；主险 `ART-6-DEDUCT`（100/100%）被批单 `END-2-DEDUCT`（500/80%）覆盖，理算 10000→7600，citations 含 `authority_rank` / `overridden_by` 与可复核 `calc_steps`；`endorsement_priority` 先批单后主险；减赔导出复用 citations / calc_steps，无人闸前 `payout_ready=false`；`machine_check` type=`sc03_endorsement_stack_reduction`。Rewrote from: REF-CASE-KB, REF-MISSIONS, REF-COURSE-04。交付提交：`ba60409`。

## Comments

- 2026-09-13：to-tickets 批准 defaults 后落盘。
- 2026-09-13：实现落盘；轨 A 全绿。
- 2026-09-13：正式关闭 — Status=resolved；原 Handoff 并入 Answer。
