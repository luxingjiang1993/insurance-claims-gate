...
SPEC：[`spec-2b-q-relay-a2a3.md`](../../../SPEC/insurance-claims-gate/spec-2b-q-relay-a2a3.md)（`SPEC-02B-Q-RELAY-A2A3` · Status=`closed`）。

前置：P-α DoD（本地 44 / GitHub #31）已 resolved。  
完成旗：仅 H8 + 本夹 DoD；**禁止**与流 P 共用深度完成旗。  
Deferred：Q-A7（pause/resume / credential proxy）不切票。  
验收：[`本项目代码/claims-gate/docs/acceptance/q-relay-a2a3-dod.md`](../../../../本项目代码/claims-gate/docs/acceptance/q-relay-a2a3-dod.md)。
...
**本波已关闭。** Frontier 为空。实现只写入 `本项目代码/claims-gate/`。未宣称 A5 / Production latch。
