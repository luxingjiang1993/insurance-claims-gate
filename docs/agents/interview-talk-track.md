...
| 口径 | 截至 2026-09-16；2b-Q 已关闭；**不**宣称 grounded / A5 / L3 / ≥300 金标 |
...
| 分 | 块 | 目标 |
|----|----|------|
| 0:00–1:00 | 开场 | 一句话说清「辅助、不替代、不出款」 |
| 1:00–3:30 | 问题与不变量 | 为什么不能把 RAG 接进终裁 |
| 3:30–7:00 | 架构主路径 | 对着图讲轨 A + 人闸 + L2 |
| 7:00–10:00 | Assist 旁路 | 可拒答、可证伪、分数≠合门禁 |
| 10:00–12:30 | Missions 中继 | A2 Schema 硬停 + A3 真 patch；非角色剧场 |
| 12:30–14:00 | 诚实边界 | 先自报缺口，再收尾 |
| 14:00–15:00 | 停顿 | 把球交给面试官 |

超时优先砍：Eval Ops 细节、H1/H2 数字、Q 测试文件名。不可砍：人闸、无 Key 绿、未宣称 A5。

---

## 0:00–1:00 开场（可背）

> 我做的是保险理赔的**条款门禁**：系统出裁决草案和条款项引用，但出款就绪必须过人闸。默认合门禁是确定性规则，不依赖大模型。AI 辅助是旁路——能拒答、引用不过门就不可采纳，分数也不冒充合规。工程上 Missions 中继做到了 Schema 硬停和真 patch，而不是角色剧场。
...
若被追问「检索好不好」：先报测量剖面——`demo_seed_eval`（向量关，可复现基线）与 `pilot_cloud_embed`（向量开，有 Key 时可对照）；**禁止**甩无剖面标签的短路 1.00 冒充语义满分。数字见一页纸 / USER_GUIDE §3.12，面试口头不必背到三位小数。
...
**收尾（可背）：**  
**权威在轨 A + 人闸；Assist 可证伪；中继诚实到 A2/A3；未宣称 A5 / grounded / 生产自动出款。**
...
| 金标呢？ | 钩子 + 薄切片协议；**不得**称 ≥300 运营 | W2 / H4 |
| 为什么不做秒赔？ | 责任争议案禁止该叙事；对外拒赔须人闸与可申诉 | PRD Won't |
| 下一步工程？ | 不是切 Q-A7；面试材料 N4–N6 时序图或 U3 README | 本 backlog |
